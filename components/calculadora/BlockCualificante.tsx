// components/calculadora/BlockCualificante.tsx
...
const defaultInputs: InputsCualificante = {
  stake: 10,
  cuotaBack: 2.0,
  cuotaLay: 2.05,
  comision: 2,
...
export default function BlockCualificante() {
  const [inputs, setInputs] = useState<InputsCualificante>(defaultInputs)

  const set = (key: keyof InputsCualificante) => (v: number) =>
    setInputs((prev) => ({ ...prev, [key]: v }))

  const result: ResultadoCualificante = calcCualificante(inputs)

  const perdidaColor =
    result.perdidaMedia > 0
      ? 'text-emerald-400'
      : result.perdidaMedia < 0
      ? 'text-red-400'
      : 'text-zinc-400'
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-3 gap-3">
        ...
        <StatCard
          label="Pérdida media"
          value={`${result.perdidaMedia >= 0 ? '+' : ''}${result.perdidaMedia.toFixed(2)} €`}
        />
      </div>
      ...
      <p className="text-xs text-zinc-500">
        💡 El objetivo del cualificante es minimizar la pérdida al desbloquear la oferta. Cuanto más
        cercanas sean las cuotas back y lay, menor será el coste.
      </p>
    </div>
  )
}
